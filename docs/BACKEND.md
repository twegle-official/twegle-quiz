...
- **`scripts/seedDetectiveCases.js`** — a one-off content seed (run manually with `node src/scripts/seedDetectiveCases.js`, reads `MONGODB_URI` from the environment same as every other script here), not wired into the boot sequence — 2 real launch cases (Rookie: "The Missing Trophy"; Junior: "The Vanishing Cupcakes"), skips a slug that already exists rather than erroring, so it's safe to re-run.

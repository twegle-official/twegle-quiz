...
A new content type + game genre — see `docs/PENDING_TASKS.md`'s dated entry for the full product/architecture reasoning (why it's simpler than both Skydrift Isles and Friendship Quiz, and the deliberate scope cuts). Backend pieces:

- **`models/DetectiveCase.js`** — one admin-authored document per case: `title`/`slug`/`description`/`intro`/`difficulty` (`rookie`/`junior`/`master`)/`estimatedMinutes`/`emoji`/`gradient`/`language`/`status`/`publishAt` (same shape every content type shares), plus nested `suspects[]` (`key`/`name`/`avatar`/`role`/`description`/`statement`), `clues[]` (`key`/`title`/`description`/`image`/`location`/`unlocksAfter: [key]`), `deductionQuestions[]` (`question`/`options`/`correctIndex`/`explanation`), `hints[]`, and one `solution` (`correctSuspectKey`/`explanation`/`provingClueKeys`). Suspects/clues reference each other by their own admin-chosen `key` string, not a Mongo `_id` — lets `unlocksAfter`/`solution` reference other parts of the same case before it's ever been saved. **No separate session model** — unlike `FriendshipInstance` or Skydrift's discovery log, there's no second player's state to keep apart from the template, so investigation progress (discovered clues, notes markers, hints used) is trusted client-side, the same trade-off `Puzzle.js`'s reveal-on-demand already makes; only the final accusation is server-verified.
- **`controllers/detectiveController.js`** — same admin/public split every content controller uses (`parsePagination`/`paginationMeta`/`logActivity`/`isValidPreviewToken`, same `slugify()` pattern as `quizController.js`). Public: `listPublishedDetectiveCases` (`PUBLIC_CASE_FIELDS` projection — no `deductionQuestions`, so a nonexistent field means nothing to fetch), `getPublishedDetectiveCaseBySlug` (adds `deductionQuestions` to the projection but strips each question's `correctIndex` via `stripDeductionAnswers()` before responding — the `solution` sub-object is excluded from the projection entirely, so it's never in the response at all, not just hidden client-side), and `solveDetectiveCase` — the one part of the flow that's actually verified: takes the player's claimed suspect/deduction-answers/clues-found/hints-used, checks the accusation and each deduction answer against the real stored `correctIndex`/`correctSuspectKey`, and computes a score server-side (`500` for the right suspect + `100` per correct deduction + up to `200` scaled by clues found, minus `30` per hint used capped at `150`) — no time component at all, deliberately, so a careful reader still scores well.
  **Caught one real bug while building this**: `getPublishedDetectiveCaseBySlug`'s field-selection string initially omitted `status`, so the live-vs-draft check (`detectiveCase.status === 'published'`) always read `undefined === 'published'` — every published case 404'd. Caught immediately via a real Supertest test (create → fetch → expect 200), not just code review; fixed by adding `status` to the projection.
- **Routes** — `GET/POST /api/detective` (public) and the matching `GET/POST/PUT/DELETE /api/admin/detective` (admin, `requireAuth`+`requireRole`), same shape as every other content type's route pair.
- **Reused, not duplicated**: added `'detectiveCase'` to `engagementController.js`'s `CONTENT_TYPES`/`MODEL_BY_TYPE` (view/share analytics for free) and to `previewController.js`'s allowed-types list (admin preview links work the same as every other content type); a new `shareDetectiveCase` in `shareController.js` (crawler-facing preview page, mirroring `shareStory`'s slug-based pattern) — deliberately never mentions the solution in its title/description, matching the public site's own "don't reveal the answer" share text.
- **`utils/levels.js`** — `POINTS_PER_DETECTIVE_CASE = 15`, added to `calculatePoints()`'s total. Kept in sync by hand with the identical frontend copy, same as every other points constant in this file.
- **`scripts/seedDetectiveCases.js`** — a one-off content seed (run manually with `node src/scripts/seedDetectiveCases.js`, reads `MONGODB_URI` from the environment same as every other script here), not wired into the boot sequence — 6 real cases (Rookie: "The Missing Trophy," "The Broken Window"; Junior: "The Vanishing Cupcakes," "The Copied Homework"; Master: "The Science Fair Sabotage," "The Talent Show Blackout" — the last 4 added 2026-09-18 to balance every difficulty tier to 2 cases each), skips a slug that already exists rather than erroring, so it's safe to re-run. Its "only run when executed directly, not when imported" guard used to build its own `file://` string by hand to compare against `import.meta.url` — broken on Windows, where a drive-letter path needs a third slash (`file:///F:/...`) that a bare `file://` prefix never adds, so the guard silently never matched and the script did nothing when run directly on this machine. Now uses `pathToFileURL` from `node:url` instead, which handles this correctly cross-platform.
- **`backend/tests/detective.test.js`** — 7 tests: a draft case is excluded from the public list, the solution/correct answers never leak in the detail response, a correct accusation with every deduction right scores exactly as the formula predicts, a wrong accusation scores lower, missing-fields and nonexistent-case requests are rejected. All passing alongside the existing 35 (36 → 42 backend tests total after a pre-existing, unrelated flaky date-boundary test in `weeklyLeaderboard.test.js` is excluded — see "Known dev-environment quirks" below, not something this feature touched).
...
Built the same day the owner created a free Cloudinary account (`console.cloudinary.com`) — see `docs/PENDING_TASKS.md`'s dated entry for the plan-pricing/multi-site-usage detail given to the owner beforehand.

- **`utils/cloudinary.js`** — configures the official `cloudinary` SDK once from 3 env vars (`CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET`), exports the configured client plus `isCloudinaryConfigured()` so the controller can give a clear "not set up yet" error instead of an opaque SDK failure if the env vars are ever missing (e.g. a fresh clone, or production before the vars are added there).
- **`controllers/uploadController.js`** / **`routes/uploadRoutes.js`** — one shared `POST /api/admin/upload/image` for every content type's image field, same "one endpoint, not one per type" reasoning `previewController.js` already established. `multer` with **in-memory storage** (never Render's own ephemeral disk — the same reasoning that ruled out a naive server-disk upload when this was originally scoped), 8MB cap, image-mimetype-only filter. The file buffer is piped straight to Cloudinary's `upload_stream` (`folder: 'twegle'`) and the real `secure_url` is handed back — nothing is ever written to this server's own filesystem at any point. Admin-only (`requireAuth` + `requireRole('superadmin', 'editor')`).
- **Wired into the two places that had a paste-a-URL placeholder waiting for it**: `Puzzle.imageUrl` and `DetectiveCase.clues[].image` — see `docs/FRONTEND.md`'s matching entry for the shared `ImageUploadField.jsx` component both forms now use. The URL text field itself is untouched and still works exactly as before (existing pasted-URL content keeps rendering) — uploading a file just fills in that same field with the returned Cloudinary URL instead of requiring one.
- **Found and fixed a real, unrelated bug while testing this**: `Engagement.js`'s `contentType` schema field never had `'detectiveCase'` added to its own `enum`, even though `engagementController.js`'s `CONTENT_TYPES`/`MODEL_BY_TYPE` already included it from the Detective feature build earlier the same day — every Detective view/share engagement call was failing Mongoose validation server-side. Silent on the frontend (a fire-and-forget call whose response is never checked), caught by a real crash in the dev server log during unrelated testing rather than proactively. Fixed by adding the missing enum value.
- **Credentials**: saved to `credentials.txt` (local-only) and `backend/.env` for local dev. **Not yet added to Render's production environment variables** — uploads will 503 with a clear "not set up yet" message on production until `CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET` are added there too (see "What's next" below).
...
- **`tests/weeklyLeaderboard.test.js`'s "last week's champion" test is real-date-dependent and can flake** — it backdates a score by a fixed number of days and asserts it lands in "last week," but that assertion's truth depends on which day of the week the test actually runs on relative to the fixed backdating offset. Seen fail on its own with no related code changed nearby — if only this one test fails and nothing touching leaderboards/scores changed, it's very likely this, not a real regression; re-run on a different day (or check the math) before spending time debugging it as a real bug.
...
## Twegle Adventure World — Phase 1: data models (2026-09-18)

A major new feature, scoped into 7 phases before any code was written — see `docs/PENDING_TASKS.md`'s dated entry for the full plan and the product reasoning. Phase 1 is data-model-only: 6 new collections, no controllers/routes/admin UI yet, so nothing about this is reachable from the live site at all yet.

- **`models/AdventureWorld.js` / `AdventureLocation.js`** — admin-authored map nodes, same `status`/`publishAt` shape every content type already shares. `mapPosition: {x, y}` is normalized 0-1, same reasoning as `SkydriftIsland.js`'s tile coordinates — the map UI can resize/reflow without ever migrating saved positions, and a new world/location can be placed anywhere from the admin panel without touching frontend layout code. `unlockRequirement` is a small discriminated sub-schema (`always` / `previousWorld` / `previousLocation` / `collectibleCount` / `challengeCount`) rather than free text, so Phase 2's progress controller can actually evaluate it server-side instead of trusting a client's claim that something is unlocked.
- **`models/AdventureChallenge.js`** — one activity per location. Two families of `type`: reused (`quiz`/`puzzle`/`game`, pointing at existing content via a generic `refId` string — a slug for quiz/game, a Puzzle's own `_id` since `Puzzle` has no slug field) so Adventure never re-implements an existing game; and 6 new mini-challenge types the site has no equivalent of yet (`find-it`/`quick-brain`/`memory`/`reaction`/`guess`/`code-breaker`), each configured via a `payload: Mixed` field instead of one schema per type, so a 7th type is addable by writing its player component + payload shape, not a migration.
- **`models/AdventureCollectible.js`** — one document per collectible *type* (a "Star" definition, not one row per star ever earned), referenced by admin-chosen `key` the same way `DetectiveCase`'s suspect/clue `key`s already work rather than a Mongo `_id`.
- **`models/AdventureCharacter.js`** — NPCs: name/avatar/role/short rotating `dialogueLines`, no dialogue tree or quest logic, per the feature's own "keep this simple" instruction.
- **`models/AdventureProgress.js`** — one per account, **account-gated** (a deliberate call, made the same way Skydrift Isles — the site's only other account-gated feature — already needed one: unlocks/collectibles need to stay server-authoritative across many separate visits, unlike Detective's client-trusted investigation state, which never needs to survive a session). Tracks `unlockedWorlds`/`unlockedLocations` (slug arrays), `completedChallenges` (ref + timestamp + optional score), running `collectibles` counts keyed by `AdventureCollectible.key`, `currentWorld`/`currentLocation` (drives the map's "you are here" marker), and a date-keyed `dailyTreasureClaimedDates` log (same `dayKey()` string format `weeklyRecap.js`/`dailyQuiz.js` already use) so a daily treasure can't be re-claimed the same day.
- **`backend/tests/adventureModels.test.js`** — 7 tests covering each model's defaults, the world→location→challenge relationships, and both challenge-type families (a reused-content `refId` and a new-type `payload`). All passing alongside the existing 44 (52/53 backend tests total, same pre-existing `weeklyLeaderboard.test.js` flake noted above).

Not yet built at the end of Phase 1: the REST endpoints, unlock-evaluation logic, and the routes themselves — see the next dated entry.

## Twegle Adventure World — Phase 2: REST endpoints (2026-09-18)

Still no admin UI or frontend — see `docs/PENDING_TASKS.md`'s dated entry for the full picture, including the open question of whether Phase 3 (frontend) or Phase 5 (admin panel) comes next.

- **`utils/adventureUnlocks.js`** — `evaluateUnlocks(progress)`, the shared pass every progress-mutating endpoint runs afterward. Fetches every published world/location/challenge once, builds in-memory lookup maps (world→its locations, location→its challenge ids), then walks each not-yet-unlocked world/location's `unlockRequirement` against the account's own progress, mutating `progress.unlockedWorlds`/`unlockedLocations` in place and returning what was newly unlocked *this call*. Deliberately brute-force rather than a database aggregation — simpler to read, and fast enough at the scale an admin would ever hand-author content at. A location's own requirement is only ever evaluated once its world is already unlocked in the same pass, so unlocking the world that contains a location and unlocking that location's own `always` requirement can both happen in one call, not two round trips.
- **`controllers/adventureController.js`** — public reads (`listWorlds`/`listLocations`/`listChallenges`/`listCollectibles`/`listCharacters`) use `optionalUserAuth` so a guest sees the same published content a logged-in visitor does, tagged with `unlocked`/`completed` flags only when there's an account to check them against — same "content is public, progress is personal" split `Bookmark`/Skydrift already follow. `getMyProgress` lazy-creates a fresh account's `AdventureProgress` on first call (same pattern `skydriftController.js`'s `getMyIsland` already uses) and runs an initial `evaluateUnlocks` pass so `always`-type content is already unlocked before the very first response. `enterLocation` moves the "you are here" map marker, rejecting a world/location the account hasn't actually unlocked — the client can never move the avatar anywhere the server doesn't independently agree is reachable. `completeChallenge` is the one genuinely server-verified action (same "everything else trusted client-side, but not this" line Detective's `solveDetectiveCase` already draws): checks the challenge and its location are both published and unlocked, is idempotent (a second completion of the same challenge never re-awards its collectible — stops button-mashing from farming rewards), awards `rewardCollectibleKey`/`rewardCollectibleCount` on first completion, then runs `evaluateUnlocks` and returns the diff. `getDailyTreasure`/`claimDailyTreasure` pick today's treasure location deterministically (`dayOfYear() % publishedLocationCount`, UTC-based) — same "everyone agrees independently by date, no stored pick, no scheduled job" pattern Horoscope/Quiz-of-the-Day/the festive banner/Detective's own case-of-the-day all already use — and reject both a location slug that isn't genuinely today's pick and a second same-day claim (`dailyTreasureClaimedDates`). The treasure's reward is a fixed `DAILY_TREASURE_COLLECTIBLE_KEY`/`_COUNT` constant for MVP simplicity — worth revisiting as admin-configurable once there's a reason to vary it day to day.
- **Routes** — `GET /api/adventure/{worlds, worlds/:slug/locations, locations/:slug/challenges, collectibles, characters, daily-treasure}` (public/`optionalUserAuth`) and `GET /me/progress` + `POST /me/enter`, `/challenges/:id/complete`, `/daily-treasure/claim` (`requireUserAuth` — Adventure's persistent progress needs a real account, unlike every guest-playable live game on the site, the same call Skydrift Isles already made). New `adventureLimiter` (`middleware/rateLimiters.js`, mirrors `skydriftLimiter`'s shape) applied to the 3 write actions; the public reads don't need one, same as every other public content list.
- **`backend/tests/adventure.test.js`** — 7 tests: public worlds list is unlocked-tagged only for a logged-in visitor, progress auto-creates with only `always`-type content unlocked, completing a challenge awards its collectible and unlocks the next world **and** its first location in one response, a repeat completion is a no-op, completing a challenge in a not-yet-unlocked location 403s, entering a not-yet-unlocked world 403s, and the daily treasure claims once and rejects a wrong-slug guess. All passing alongside the existing 52 (59/60 backend tests total, same pre-existing `weeklyLeaderboard.test.js` flake noted above).
...
Folds Adventure challenge completion into the site's one shared points/levels/badges system rather than leaving it as its own separate reward track — see `docs/FRONTEND.md`'s matching entry for the client side of this.

- **`controllers/adventureController.js`** — `completeChallenge` now also returns `rewardPoints`: the completed challenge's own admin-set `rewardPoints` field when it's been set (`> 0`), otherwise a flat `POINTS_PER_ADVENTURE_CHALLENGE` default (imported from `utils/levels.js`) so a challenge seeded before this phase existed — still at the model's own `0` default — doesn't silently award nothing forever. Returns `0` when `alreadyCompleted` is true, so replaying an already-solved challenge can't be used to farm points either, the same idempotency guarantee that already applied to its collectible award.
- **`utils/levels.js`** (both this backend copy and the hand-mirrored `frontend/src/utils/levels.js`) — new `POINTS_PER_ADVENTURE_CHALLENGE = 4` export (the fallback described above) and `calculatePoints()` now adds `stats.adventurePointsEarned` straight in. Unlike every other stat here, this one isn't a flat-rate-times-count — a completed challenge's own `rewardPoints` genuinely varies per challenge, admin to admin — so the running total is pre-summed client-side as each completion lands (see `FRONTEND.md`) rather than levels.js multiplying a count by a constant.
- **Found and fixed one more real, unrelated bug while wiring this up**: `frontend/src/utils/statsSync.js`'s `mergeStats()` — the function that reconciles a device's local stats with the account's server copy on every login/poll — builds its return value as an object literal that only lists fields it explicitly names, so any stat left off that list is silently dropped every time two devices' stats get merged. All 4 of Detective's own stat fields (`detectiveCasesSolved`, `detectiveScores`, `detectivePerfectInvestigations`, `detectiveMasterCasesSolved`) had been missing from that list since Detective shipped — a second device's Detective progress could be quietly lost on merge. Fixed by adding all 4, plus the 2 new Adventure fields this phase introduces, so the exact same gap wasn't just reintroduced for Adventure too.
- New tests in `adventure.test.js`: the flat-default-fallback case, and a challenge with its own `rewardPoints` set returning that value instead. 69/70 backend tests passing (1 pre-existing, unrelated `weeklyLeaderboard.test.js` date-boundary flake).
- Verified live on production twice — once via a scripted API call (seeded a challenge with `rewardPoints: 25`, confirmed the completion response returned exactly `25`, confirmed `adventurePointsEarned`/`adventureChallengesCompleted` round-tripped correctly through the stats save/fetch endpoints) and once through the real browser UI with a second throwaway account (signed up, played a seeded `guess` challenge, confirmed the celebration banner, then confirmed the Badges page immediately reflected the new points total and the new badge). Full details in `docs/PENDING_TASKS.md`'s dated entry. All seeded content and both test accounts deleted afterward.

## Twegle Adventure World — Phase 6: analytics (2026-09-18)

"Opens/completion-rate/most-visited," per the feature's own original scoping — built as two separate pieces, since they're genuinely different kinds of signal with different existing infrastructure to reuse. See `docs/FRONTEND.md`'s matching entry for both admin-panel-side tables.

- **"Opens"/"most-visited"** reuse the site's existing generic `Engagement` model wholesale, the same anonymous view/share tracking every other content type (quizzes, posts, Detective cases, ...) already has — no new tracking system for Adventure specifically. **`models/Engagement.js`** — added `'adventureLocation'` to `contentType`'s enum. **`controllers/engagementController.js`** — added `adventureLocation: AdventureLocation` to `MODEL_BY_TYPE` and the type to `CONTENT_TYPES`; also replaced the old single `contentType === 'puzzle' ? 'question' : 'title'` inline ternary in `getEngagementSummary` with a small `TITLE_FIELD_BY_TYPE` map (now `{ puzzle: 'question', adventureLocation: 'name' }`, since `AdventureLocation`'s display field is `name`, not `title`) — same underlying idea, just needed to scale past 2 special cases. No route changes needed: `POST /api/engagement` and `GET /api/admin/engagement/:contentType` already handle any listed `contentType` generically.
- **"Completion rate"** has no existing infrastructure to reuse — no other content type on the site tracks a comparable "did the visitor actually finish this" signal (Detective's own solve tracking lives in stats, not a queryable engagement-style table). New **`getAdventureAnalytics`** in `controllers/adminAdventureController.js` (`GET /api/admin/adventure/analytics`, same `requireRole('superadmin'|'editor'|'analyst')` read access every other admin analytics endpoint already uses) aggregates `AdventureProgress` directly with two `$unwind`+`$group` pipelines: how many accounts have `unlockedWorlds` containing each world's slug, and how many have a `completedChallenges` entry referencing each challenge — both expressed as a rate out of `AdventureProgress.countDocuments()` (every account that's ever played Adventure at all). Deliberately the simplest honest denominator available: "of all Adventure players," not "of players who specifically reached this world/challenge," since reaching one isn't tracked as its own event — the response is explicit about this and the admin page states it directly rather than implying more precision than the data actually supports.
- **Found one more real, unrelated bug while verifying this live**: an admin's `deleteEndUser` (`controllers/adminEndUserController.js`) only ever deletes the `EndUser` document itself, never any related account-linked collection — `AdventureProgress` included. Every test account created and deleted during this whole session's Phase 3/4/6 verification passes left an orphaned `AdventureProgress` row behind, which skewed this phase's own analytics numbers (an inflated `totalPlayers`) until noticed mid-verification. Not fixed here — it's a pre-existing, feature-agnostic data-hygiene gap affecting every account-linked collection (Skydrift included), not something this phase introduced — flagged as its own follow-up task instead of folded into this one.
- New tests: `adventureAdmin.test.js` gets a test seeding 4 `AdventureProgress` documents by hand (3 unlocking a world, 2 of those completing a challenge) and asserting the endpoint's exact counts/rates, plus a read-only-analyst-can-read-this-too check; `adventure.test.js` gets a test posting a real `POST /api/engagement` view for an `adventureLocation` and confirming it surfaces correctly through `GET /api/admin/engagement/adventureLocation`. 72/73 backend tests passing (1 pre-existing, unrelated `weeklyLeaderboard.test.js` date-boundary flake).
- Verified live on production: seeded a real world/location/challenge via the admin API, created 3 throwaway accounts (2 completed the challenge), posted 2 real "view" engagements against the location, and confirmed `getAdventureAnalytics`/the engagement summary both returned the exact right numbers; then redid the "opens" half through an actual browser session (signed up, visited the location page for real) and confirmed the admin Analytics page's new table showed that real view. All seeded content and every test account deleted afterward — confirmed back to 0 worlds on production.

## Twegle Adventure World — Phase 7: real launch content (2026-09-19)

The last of the 7 phases — real worlds/locations/challenges written and left published, replacing every prior phase's throwaway test/demo content with the genuine article. No code changes; this is a content-authoring pass via the admin API (same scripted-seeding approach used for Twegle Detective's own launch cases).

- **2 worlds**: 🏠 Twegle Town (`always` unlocked, the entry point) and 🏫 Mystery School (`previousWorld`, unlocks once Twegle Town hits 100%).
- **5 locations**: Town Square + Twegle Park in Twegle Town (a straight `previousLocation` chain); Library, Science Lab, and Puzzle Room in Mystery School (same chain, 3 deep).
- **10 challenges**, a genuine mix of every built type rather than leaning on just one or two: 2 reused `quiz` challenges (linking real, already-published quizzes by slug), 1 reused `game` challenge (linking a real game by slug), 1 reused `puzzle` challenge (linking a real, already-published riddle by its own `_id`), 2 `guess` riddles, 2 `quick-brain` brain-teasers, and 2 `code-breaker` ciphers (simple Caesar-shift puzzles, e.g. "QBSL" shifted back by 1 decodes to "park") — every payload hand-verified against its own answer before seeding, not just written and trusted.
- **4 collectibles**: star/gem (common), key (rare), mystery-piece (epic, the Puzzle Room's capstone reward) — real names/icons/descriptions, not placeholder text.
- **2 characters**: Toby the Twegle Fox (Twegle Town's guide) and Professor Quill (Mystery School's), each with 3 rotating dialogue lines.
- Seeded via a one-off Node script against the live admin API (same pattern Detective's `seedDetectiveCases.js` established, though this one wasn't committed to the repo since Adventure content is meant to be admin-authored going forward, not re-seeded from a script on every deploy).
- Verified with a full scripted playthrough against production using a real, since-deleted test account: signed up, confirmed Twegle Town started unlocked and Mystery School locked, completed all 3 Town Square challenges (confirming Twegle Park unlocked after the first one), completed both Twegle Park challenges including decoding the real cipher (confirming Mystery School unlocked the moment Twegle Town hit 100%), then chained through Library → Science Lab → Puzzle Room the same way, decoding the second real cipher along the way, and confirmed the final progress showed all 10 challenges completed and exactly the right collectible counts (10 stars, 6 gems, 2 keys, 1 mystery piece) matching the authored rewards by hand. Also verified through the real browser UI: logged into the same test account, confirmed the Adventure Map, Progress page, and the Puzzle Room's "✓ Done" state all rendered correctly with the real content. Only the throwaway test account was deleted afterward — every world/location/challenge/collectible/character seeded this phase was left published, confirmed still live on production after the account cleanup.
- **Found one more real, unrelated bug while cleaning up test accounts across this whole session's Adventure verification passes**: an admin's `deleteEndUser` doesn't cascade-delete `AdventureProgress` (or other account-linked collections) — see Phase 6's entry above for the detail; flagged as its own follow-up task, not fixed here.
...
- **Add the 3 Cloudinary env vars to Render's production environment** (`CLOUDINARY_CLOUD_NAME`/`CLOUDINARY_API_KEY`/`CLOUDINARY_API_SECRET`, values in `credentials.txt`) — real image uploads are built and working locally, but won't work on the live site until these are set there too.

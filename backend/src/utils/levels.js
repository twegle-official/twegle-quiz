...
// every kind of activity already tracked in EndUser.stats (see statsSync.js
// on the frontend for the exact blob shape this reads). Kept in sync by hand
// with the identical copy in frontend/src/utils/levels.js: the frontend uses
// it to show "your level" from local/synced stats, this copy lets the public
// leaderboard compute everyone's points server-side without trusting a
// client-submitted number. Weights are a first-pass balance, easy to retune
// later — deliberately not tied to any single activity type, so no name below
// implies "quiz only" or "games only."
...
// `stats` is the badge-counters object (EndUser.stats.stats), `streakCount`
// is EndUser.stats.streak?.count — see statsSync.js for why the nesting.

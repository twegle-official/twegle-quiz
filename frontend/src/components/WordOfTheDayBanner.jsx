  ...
  if (variant === 'rail') {
    ...
    return (
      <Link
        ...
        className={`flex flex-row-reverse items-center rounded-l-xl shadow-lg overflow-hidden bg-gradient-to-r from-green-500 to-emerald-600 text-white transition-[width] duration-300 ease-out ${
      ...
      >
        {/* 🔤 here specifically (not the 🟩 Wordle-tile color used
            ...
            WordOfTheDay.jsx's own header already uses. Mobile (`controlled`)
            is sized smaller than desktop's hover rail — feedback was
            specifically about the mobile version feeling oversized,
            desktop's own size was fine as-is. */}
        ...
        <div className={`min-w-0 flex-1 pr-3 ${controlled ? 'pl-2' : 'pl-3'}`}>
          <p className={`font-bold uppercase tracking-wide text-white/80 truncate ${controlled ? 'text-[8px]' : 'text-[9px]'}`}>🔥 Word Streak</p>
          <p className={`font-bold truncate ${controlled ? 'text-xs' : 'text-sm'}`}>Twegle Word #{getDayNumber()}</p>
        </div>
      </Link>
    )
  }

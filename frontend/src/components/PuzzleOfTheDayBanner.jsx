  ...
  if (variant === 'rail') {
    ...
    return (
      <Link
        ...
        className={`flex flex-row-reverse items-center rounded-l-xl shadow-lg overflow-hidden bg-gradient-to-r from-violet-500 to-indigo-500 text-white transition-[width] duration-300 ease-out ${
      ...
      >
        ...
        <div className={`min-w-0 flex-1 pr-3 ${controlled ? 'pl-2' : 'pl-3'}`}>
          <p className={`font-bold uppercase tracking-wide text-white/80 truncate ${controlled ? 'text-[8px]' : 'text-[9px]'}`}>🔥 Puzzle Streak</p>
          <p className={`font-bold truncate ${controlled ? 'text-xs' : 'text-sm'}`}>{puzzle.question}</p>
        </div>
      </Link>
    )
  }

...
import MobileStreakRail from '../components/MobileStreakRail'
...
export default function Home() {
  ...
  return (
    <div>
      ...
      {/* Today's Mystery + the 3 daily streaks now live in a floating
          right-edge rail at every width — hover-driven at `xl`+
          (DailyStreakRail.jsx), tap-driven below that (MobileStreakRail.jsx,
          since there's no hover on a touchscreen). Moved off the inline
          position entirely (used to render full-width banners here on
          mobile/tablet too) so the main content grid starts sooner at every
          width, not just desktop. */}
      <DailyStreakRail language={language} quizzes={allQuizzes} puzzles={allPuzzles} />
      <MobileStreakRail language={language} quizzes={allQuizzes} puzzles={allPuzzles} />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {displayedItems.map((game, i) => (
            <div
              ...
              style={{ animationDelay: `${i * 60}ms`, animationFillMode: 'backwards' }}
            >
              <GameCard game={game} filterMode={gameCategory} language={language} />
            </div>
          ))}
        </div>
      )}

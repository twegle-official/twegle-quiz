    ...
    <div>
      ...
      {entries && (
        <div className="bg-white rounded-xl shadow-sm divide-y divide-gray-100">
          {entries.length === 0 && (
            <p className="p-6 text-gray-400 text-center">No feedback yet.</p>
          )}
          {entries.map((entry) => (
            <div
              key={entry._id}
              className={`flex items-start justify-between gap-4 p-4 ${entry.read ? '' : 'bg-violet-50/40'}`}
            >
              <div className="min-w-0">
                <p className="text-xs text-gray-400 mb-1 flex items-center gap-2 flex-wrap">
                  ...
                  {!entry.read && (
                    <span className="px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 font-semibold">New</span>
                  )}
                  {entry.contentType && (
                ...
                {entry.contentType && (
                  <p className="text-sm text-gray-500 mb-1">
                    {entry.contentType === 'quiz' ? 'Quiz' : entry.contentType === 'story' ? 'Story' : 'Post'}: <span className="font-medium text-gray-700">{entry.contentLabel || '(untitled)'}</span>
                  </p>
                )}
                <p className="text-gray-900 whitespace-pre-line">{entry.message}</p>
              </div>
              {canWrite && (

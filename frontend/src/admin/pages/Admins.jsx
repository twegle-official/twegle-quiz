  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-[360px_1fr] gap-6 items-start">
        <form onSubmit={handleCreate} className="bg-white rounded-xl shadow-sm p-5 space-y-3">
          <h2 className="font-semibold text-gray-900 mb-2">Add a new admin</h2>
          <input
            required
            placeholder="Name"
            value={form.name}
        ...
        {admins && (
          <div className="bg-white rounded-xl shadow-sm divide-y divide-gray-100">
            {admins.length === 0 && (
              <p className="p-6 text-gray-400 text-center">No other admins yet.</p>
            )}
            {admins.map((admin) => (

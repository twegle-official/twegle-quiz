  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-3xl">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-5 mb-6">
        <h2 className="font-bold text-gray-900 dark:text-gray-100 mb-3">Results</h2>
        ...
        <div className="space-y-4">
          {quiz.results.map((result, i) => (
            <div key={i} className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-gray-700 rounded-lg p-3 space-y-2">
              <div className="flex flex-wrap gap-2">
                <input
                  placeholder="key (e.g. oily)"
                  value={result.key}
                  onChange={(e) => updateResult(i, 'key', e.target.value)}
                  className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 w-28 px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm"
                />
                <input
                  placeholder="emoji"
                  value={result.emoji}
                  onChange={(e) => updateResult(i, 'emoji', e.target.value)}
                  className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 w-14 px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-center"
                />
                <input
                  placeholder="Title"
                  value={result.title}
                ...
                {quiz.type === 'trivia' && (
                  <>
                    <input
                      ...
                      className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 w-24 px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm"
                    />
                    <input
                      type="number"
                      min="0"
              ...
              <textarea
                ...
                className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm"
                rows={2}
              />
            </div>
          ))}
      ...
      {/* Questions */}
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-5 mb-6">
        <h2 className="font-bold text-gray-900 dark:text-gray-100 mb-3">Questions</h2>
        <div className="space-y-5">
          {quiz.questions.map((question, qIndex) => (
            <div key={qIndex} className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-gray-700 rounded-lg p-3 space-y-2">
              <div className="flex flex-wrap gap-2">
                <input
              ...
              <div className="space-y-2 pl-3">
                {question.options.map((option, oIndex) => (
                  <div key={oIndex} className="flex flex-wrap gap-2">
                    ...
                    />
                    {quiz.type === 'trivia' ? (
                      <label className="flex items-center gap-1.5 px-2 py-1.5 text-sm shrink-0 text-gray-600 dark:text-gray-400">
                        <input
                          type="checkbox"
                      ...
                      <select
                        ...
                        className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm shrink-0"
                      >
